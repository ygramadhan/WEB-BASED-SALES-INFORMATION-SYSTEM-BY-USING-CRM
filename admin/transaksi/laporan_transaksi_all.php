...
while($data_order=mysqli_fetch_array($order)){
    ...
    $total=$total+$data_order['jml_byr'];
        ...
        if($pdf->GetStringWidth($data_order['alamat']) < $cellWidth){
            ...
            $textLength=strlen($data_order['alamat']);	//total panjang teks
            ...
            while($startChar < $textLength){ //perulangan sampai akhir teks
                ...
                ($startChar+$maxChar) < $textLength ) {
                    $maxChar++;
                    $tmpString=substr($data_order['alamat'],$startChar,$maxChar);
                }
        ...
        $cellWidth1=35; //lebar sel
        $cellHeight1=7; //tinggi sel satu baris normal
        
        if($pdf->GetStringWidth($data_produk['nama_produk']) < $cellWidth1){
            //jika tidak, maka tidak melakukan apa-apa
            $line1=1;
        }else{
            //jika ya, maka hitung ketinggian yang dibutuhkan untuk sel akan dirapikan
            ...
            $textLength1=strlen($data_produk['nama_produk']);	//total panjang teks
            $errMargin1=2;		//margin kesalahan lebar sel, untuk jaga-jaga
            ...
            while($startChar1 < $textLength1){ //perulangan sampai akhir teks
                ...
                ($startChar1+$maxChar1) < $textLength1 ) {
                    $maxChar1++;
                    $tmpString1=substr($data_produk['nama_produk'],$startChar1,$maxChar1);
                }
                //pindahkan ke baris berikutnya
