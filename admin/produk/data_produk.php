           ...
           <div class="container">
            ...
            if($grid=='grid'){
                ...
                <div class="biolife-tab biolife-tab-contain sm-margin-top-34px">
                    ...
                    ?>
                    <a href="index.php?page=produk&view=grid" class="btn btn-primary" ><i class="fa fa-eye fa-sm text-white-60"></i> Lihat Katalog</a>
                    <a href="index.php?page=add_produk" class="btn btn-primary" ><i class="fa fa-plus fa-sm text-white-60"></i> Tambah Data</a>
                    <br><br>
                        <table class="striped">
                            <thead>
                            ...
                            while($data_produk = mysqli_fetch_array($produk)){
                                ...
                                if($hit_status<3) {
                                    if($data_produk['status']==0) {
                                        $status = "<a href='index.php?page=up_produk&id=".$data_produk['kd_produk']."' class='btn btn-success'><i class='fa fa-level-up fa-sm text-white-60'></i>  Naikan</a>";
                                    } else {
                                        $status = "<a href='index.php?page=down_produk&id=".$data_produk['kd_produk']."' class='btn btn-success'><i class='fa fa-level-down fa-sm text-white-60'></i>Turunkan</a>";
                                    }
                                } else {
                                    if($data_produk['status']==0) {
                                        $status = "<a href='#' class='btn btn-danger'><i class='fa fa-times fa-sm text-white-60'></i> Naikan</a>";
                                    } else {
                                        $status = "<a href='index.php?page=down_produk&id=".$data_produk['kd_produk']."' class='btn btn-success'><i class='fa fa-level-down fa-sm text-white-60'></i>Turunkan</a>";
                                    }
                                }
                            ...
                            <tr>
                                ...
                                <td><?=$status?></td>
                                <td>
                                <a href="index.php?page=lihat_produk&id=<?=$data_produk['kd_produk']?>" class="btn btn-info" ><i class="fa fa-eye fa-sm text-white-60"></i></a>
                                <a href="index.php?page=edit_produk&id=<?=$data_produk['kd_produk']?>" class="btn btn-warning" ><i class="fa fa-pencil fa-sm text-white-60"></i></a>
                                <a href="index.php?page=delete_produk&id=<?=$data_produk['kd_produk']?>" class="btn btn-danger" ><i class="fa fa-trash fa-sm text-white-60"></i></a></td>
                            </tr>
                            <?php
                            }
