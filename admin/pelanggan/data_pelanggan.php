                        ...
                        <table class="striped">
                            ...
                            while($data_pelanggan=mysqli_fetch_array($pelanggan)){
                                ...
                                if($data_pelanggan['status_pelanggan']==0){
                                    $status = "<a href='index.php?page=konfirmasi_pelanggan&id=".$data_pelanggan['id_pelanggan']."' class='btn btn-success'>Konfirmasi</a>";
                                } else {
                                    $status= "Aktif";
                                }

                                $chat = mysqli_query($koneksi, "SELECT * FROM tbl_chat WHERE id_pelanggan='$data_pelanggan[id_pelanggan]' AND sender='Pelanggan' AND status='0'");
                            ...
                            <tr>
                                ...
                                <a href="index.php?page=edit_pelanggan&id=<?=$data_pelanggan['id_pelanggan']?>" class="btn btn-warning" ><i class="fa fa-pencil fa-sm text-white-60"></i></a>
                                <a href="index.php?page=delete_pelanggan&id=<?=$data_pelanggan['id_pelanggan']?>" class="btn btn-danger" ><i class="fa fa-trash fa-sm text-white-60"></i></a></td>
                                <td>
                                    <div class="biolife-cart-info">
                                        <div class="wishlist-block hidden-sm hidden-xs">
                                            <span class="icon-qty-combine">
                                            <a href="index.php?page=chat&id=<?=$data_pelanggan['id_pelanggan']?>" class="btn btn-dark" ><i class="fa fa-comments fa-sm text-white-60"></i><span class="qty"><?=$jml_chat?></span></a>
                                            </span>
                                        </div>
                                    </div>
                                </td>
